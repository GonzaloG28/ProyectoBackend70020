import productManage from "./productManage"

//{ title, description, code, price, stock, category, thumbnails }

//añadimos dos productos para probar que este funcionando
productManage.addProduct({
    title: "El Monje que Vendió su Ferrari",
    description: "historia de Julian Mantle, un superabogado cuya vida estresante, desequilibrada y obsesionada con el dinero acaba provocándole un infarto.",
    code: "AAA1",
    price: 2000,
    stock: 14,
    category: "Libro",
    thumbnails: "https://images.cdn3.buscalibre.com/fit-in/360x360/33/d6/33d6e00def7c190c019e6e244c23a683.jpg"
})

productManage.addProduct({
    title: "Harry Potter: Un Año Mágico",
    description: "Que la magia de Harry Potter te acompañe todos los días del año. Las ediciones ilustradas de Jim Kay han embrujado a toda una generación de lectores de Harry Potter.",
    code: "AAA2",
    price: 4000,
    stock: 6,
    category: "Libro",
    thumbnails: "https://images.cdn2.buscalibre.com/fit-in/360x360/53/cd/53cd2f9bdea6cb278bce6acaaaaa9206.jpg"
})